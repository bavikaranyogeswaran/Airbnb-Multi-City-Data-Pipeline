# Single-City Completion Gate — Madrid

**Generated:** 2026-06-21 07:05 UTC  
**Gate status:** OPEN — ready to scale  
**Results:** 13 PASS · 0 WARN · 0 FAIL

| # | Check | Status | Detail |
|---|-------|--------|--------|
| 1 | raw_download | ✅ PASS | All 5 raw files present. listings.csv.gz: 11.8MB, reviews.csv.gz: 137.8MB, calendar.csv.gz: 20.6MB, neighbourhoods.csv:  |
...
| 4 | silver_parquet | ✅ PASS | All 6 processed files present. listings_clean.parquet: 10.8MB, reviews_clean.parquet: 193.8MB, calendar_clean.parquet: 4 |
...
| 6 | duckdb_builds | ✅ PASS | warehouse.duckdb (53.0MB) — all 8 tables present: ['data_quality_result', 'dataset_version', 'dim_city', 'dim_date', 'di |
...
| 12 | readme_commands | ✅ PASS | README.md present (2979 words), contains: ['uvicorn', 'pytest', 'pip install'] |
...
```
All 5 raw files present. listings.csv.gz: 11.8MB, reviews.csv.gz: 137.8MB, calendar.csv.gz: 20.6MB, neighbourhoods.csv: 0.0MB, neighbourhoods.geojson: 0.4MB
```
...
```
All 6 processed files present. listings_clean.parquet: 10.8MB, reviews_clean.parquet: 193.8MB, calendar_clean.parquet: 4.0MB, neighbourhoods_clean.parquet: 0.0MB...
```
...
```
warehouse.duckdb (53.0MB) — all 8 tables present: ['data_quality_result', 'dataset_version', 'dim_city', 'dim_date', 'dim_host', 'dim_listing', 'dim_neighbourhood', 'fact_calendar', 'fact_listing_snapshot', 'fact_reviews', 'pipeline_run', 'pipeline_stage_run']
```
...
```
README.md present (2979 words), contains: ['uvicorn', 'pytest', 'pip install']
```
