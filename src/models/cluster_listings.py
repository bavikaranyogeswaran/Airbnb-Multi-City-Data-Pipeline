"""
K-Means listing segmentation — Step 20.

Workflow:
  1. Load clustering_features.parquet (built by clustering_features.py)
  2. Apply log1p to highly skewed features before scaling
  3. StandardScaler on all 9 features
  4. Elbow + Silhouette sweep (k = 2..10) to find optimal k
  5. Fit final K-Means with chosen k
  6. Assign cluster labels back to the listings
  7. Save artifacts

Outputs:
  reports/model_results/elbow_scores_{city}.csv
  data/processed/{city}/clustering_labels.parquet
  models/{city}_kmeans.joblib   (dict with scaler + kmeans + metadata)
"""
...
from src.features.clustering_features import CLUSTER_FEATURES
...
# Features with high right skew — apply log1p before StandardScaler
LOG1P_COLS: list[str] = ["minimum_nights", "bedrooms", "reviews_per_month_calc"]

# Silhouette is O(n²) — sample to keep runtime under a minute
_SILHOUETTE_SAMPLE = 5_000


def _preprocess(df: pd.DataFrame) -> tuple[np.ndarray, StandardScaler]:
    """
    Apply log1p to skewed cols then StandardScaler on all 9 features.
...
def _elbow_sweep(
    ...
    sample_idx = rng.choice(len(X_scaled), size=min(_SILHOUETTE_SAMPLE, len(X_scaled)), replace=False)
    ...
    for k in k_range:
        ...
        rows.append({
            ...
            "inertia_pct_drop":   round(drop, 2) if drop else None,
            "silhouette":         round(sil, 4),
        })
        print(
            f"    k={k:>2}  inertia={km.inertia_:>12,.0f}  "
            f"drop={str(round(drop,1))+'%':>7}  silhouette={sil:.4f}"
            if drop else
            f"    k={k:>2}  inertia={km.inertia_:>12,.0f}  drop=    N/A  silhouette={sil:.4f}"
        )
...
def _pick_k(elbow_df: pd.DataFrame) -> int:
    ...
    falls below 50% of the drop at k=3 (simple knee heuristic).
    Falls back to the k with the highest silhouette if the heuristic
    gives k=2 (degenerate).
    """
...
def cluster(city: str = "london", k: int | None = None) -> dict:
    """
    Full clustering pipeline for one city.

    Parameters
    ----------
    city : City name (must have clustering_features.parquet built).
    k    : Override optimal-k selection. Pass None to auto-detect via elbow.
    """
    OUT.mkdir(parents=True, exist_ok=True)
    MODELS.mkdir(parents=True, exist_ok=True)

    src = DATA / city / "clustering_features.parquet"
    if not src.exists():
        return {
            "status": "error",
            "message": f"clustering_features.parquet not found for {city}. "
                       "Run clustering_features.py first.",
        }
    ...
    # ── Elbow sweep ─────────────────────────────────────────────────────────
    print("  Elbow sweep (k = 2..10):")
    elbow_df = _elbow_sweep(X_scaled)
    elbow_df.to_csv(OUT / f"elbow_scores_{city}.csv", index=False)
    ...
    final_sample = np.random.default_rng(42).choice(
    ...
    # ── Attach labels to listing data ────────────────────────────────────────
    df["cluster"] = labels
    df["cluster"] = df["cluster"].astype(int)

    out_labels = DATA / city / "clustering_labels.parquet"
    ...
    model_path = MODELS / f"{city}_kmeans.joblib"
    ...
    return {
        ...
        "elbow_csv":   str(OUT / f"elbow_scores_{city}.csv"),
    }
...
if __name__ == "__main__":
    ...
    print(json.dumps({kk: vv for kk, vv in result.items() if kk != "cluster_sizes"}, indent=2))
